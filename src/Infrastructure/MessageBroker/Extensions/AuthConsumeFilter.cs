...
where T : class
{
    ...
    public Task Send(ConsumeContext<T> context, IPipe<ConsumeContext<T>> next)
    {
        ...
        try
        {
            claimsPrincipal = _jwtTokenService.GetPrincipalFromToken(accessToken, true);
        }
        catch (Exception ex)
        {
            _logger.LogDebug("Token validation failed in AuthConsumeFilter: {Message}", ex.Message);
        }
